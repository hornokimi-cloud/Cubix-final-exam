...
def halmaz_kerdes(kerdes):
    ...
    beirt_valaszok = [darab.strip() for darab in valasz.split(",") if darab.strip()]
    eredeti_beirt_valaszok = {tisztit(darab): darab for darab in beirt_valaszok}
    valaszok = set(eredeti_beirt_valaszok)
    ...
    felesleges = valaszok - helyes

    if valaszok == helyes:
        return kerdes["pont"], "Minden szükséges elemet felsoroltál."

    helyes_valaszok_szama = len(talalatok) - len(felesleges)
    helyes_valaszok_szama = max(0, helyes_valaszok_szama)
    reszpont = kerdes["pont"] * helyes_valaszok_szama / len(helyes)
    ...
    if felesleges:
        felesleges_szavak = [eredeti_beirt_valaszok[elem] for elem in felesleges]
        visszajelzes += ". Felesleges vagy hibás válasz: " + ", ".join(sorted(felesleges_szavak))
